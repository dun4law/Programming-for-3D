...
public class PlaneAnimation : MonoBehaviour
{
    ...
    {
        if (missileGraphics == null || index < 0 || index >= missileGraphics.Count)
            return;
        if (missileGraphics[index] != null)
            missileGraphics[index].SetActive(visible);
    }

    public void SetMissileGraphics(List<GameObject> graphics)
    {
        missileGraphics = graphics ?? new List<GameObject>();

        foreach (var g in missileGraphics)
        {
            if (g != null)
                g.SetActive(true);
        }
    ...
    void UpdateControlSurfaces(float dt)
    {
        ...
        rightAileron.localRotation = CalculatePose(
            rightAileron,
            Quaternion.Euler(deflection.z * maxAileronDeflection, 0, 0)
        );
        leftAileron.localRotation = CalculatePose(
            leftAileron,
            Quaternion.Euler(-deflection.z * maxAileronDeflection, 0, 0)
        );
        ...
        foreach (var t in elevators)
        {
            t.localRotation = CalculatePose(
                t,
                Quaternion.Euler(deflection.x * maxElevatorDeflection, 0, 0)
            );
        }
        ...
        foreach (var t in rudders)
        {
            t.localRotation = CalculatePose(
                t,
                Quaternion.Euler(0, -deflection.y * maxRudderDeflection, 0)
            );
        }
    }
    ...
    void UpdateAirbrakes(float dt)
    {
        ...
        airbrake.localRotation = CalculatePose(
            airbrake,
            Quaternion.Euler(-airbrakePosition * airbrakeDeflection, 0, 0)
        );
    }
    ...
    void UpdateFlaps(float dt)
    {
        ...
        foreach (var t in flaps)
        {
            t.localRotation = CalculatePose(
                t,
                Quaternion.Euler(flapsPosition * flapsDeflection, 0, 0)
            );
        }
    }
    ...
    public void DisableAfterburners()
    {
